...
def parse_args():
    parser = argparse.ArgumentParser(description="Run DAICF.")
    ...
    parser.add_argument('--batch_choice', nargs='?', default='fixed:256',
    ...
    parser.add_argument('--regs', nargs='?', default='[1e-6,1e-6,1e-6]',
                        help='Regularization for user and item embeddings.')
    parser.add_argument('--reg_W', nargs='?', default='[15,15,15]',
...
class DAICF:
    ...
    def _add_embedding_matrix(self):
        dataset = Dataset(args.path + args.dataset)

        self.user_item_embedding = tf.convert_to_tensor(dataset.ratingMatrix)
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):  # The embedding initialization is unknown now
            ...
            # embedding for neighbor infor
            self.embedding_n = tf.Variable(tf.truncated_normal(shape=[self.num_users, self.embedding_size], mean=0.0, stddev=0.01), name='embedding_n', dtype=tf.float32, trainable=trainable_flag)
            ...
            # Variables for DAICF
            self.weights = {
                'out': tf.Variable(tf.random_normal([self.n_hidden[-1]*2, 1], mean=0, stddev=np.sqrt(2.0 / (self.n_hidden[-1] + 1))), name='weights_out')
            }
            self.biases = {
                'out': tf.Variable(tf.random_normal([1]), name='biases_out')
            }
            n_hidden_0 = self.embedding_size*2
    ...
    def _create_inference(self):
        with tf.name_scope("inference"):
            ...
            self.item_neighbor = tf.nn.embedding_lookup(self.item_user_embedding,self.item_input)  # (b,1,num_user)
            self.item_neighbor = tf.matmul(self.item_neighbor,self.embedding_n)  # (b,1,e)
            ...
            # self.embedding_p = tf.reduce_sum(self.embedding_q_, 1)
            ...
            self.item_neighbor = tf.reduce_sum(self.item_neighbor,1)  # (b,e)

            # DAICF
            fm = tf.multiply(self.embedding_p,self.item_neighbor) # (b,e)
            layer1 = tf.concat([self.embedding_p,self.item_neighbor],axis=1)  #(b,2e)
            ...
            concat = tf.concat([fm, layer1], 1)  # (b,2e)
            out_layer = tf.matmul(concat, self.weights['out']) + self.biases['out']  # (?, 1)
    ...
    def _create_loss(self):
        with tf.name_scope("loss"):
            self.loss = tf.losses.log_loss(self.labels, self.output) + \
                        ...
                        self.lambda_bilinear * tf.reduce_sum(tf.square(self.embedding_n))
    ...
    def _summary(self):
        ...
        self.write = tf.summary.FileWriter('./graph/DAICF',tf.get_default_graph())
        with tf.name_scope("summaries"):
            tf.summary.scalar('loss', self.loss)
            tf.summary.histogram('his loss',self.loss)
            self.summary_op = tf.summary.merge_all()

    def build_graph(self):
        self._add_embedding_matrix()
...
def training(flag, model, dataset, epochs, num_negatives):
    ...
    weight_path = 'Pretraining/%s/' % (model.dataset_name)
    saver = tf.train.Saver({'c1':model.c1})
    ...
    with tf.Session() as sess:
        ...
        if flag != 0:
            ...
            p_c1, p_e_Q, p_e_n, p_bias, p_w, p_b, p_h = sess.run(
                [model.c1, model.embedding_Q, model.embedding_n, model.bias, model.W, model.b, model.h])
            ...
            # model.embedding_n = tf.Variable(p_e_n, dtype=tf.float32, trainable=True, name='embedding_n')
            model.bias = tf.Variable(p_bias, dtype=tf.float32, trainable=True, name='embedding_Q')
            ...
            model.h = tf.Variable(p_h, name='H_for_MLP', dtype=tf.float32, trainable=True)
        ...
        for epoch_count in range(epochs):
            ...
            if epoch_count % model.verbose == 0:
                ...
                result.append([epoch_count,batch_time,train_time,hr,ndcg,test_loss,eval_time,train_loss,loss_time])
        ...
        # save_path = 'Pretraining/%s' % (model.dataset_name)
        # saver.save(sess, save_path, global_step=epoch_count)
        # print("Saving model embedding to %s" % save_path)
...
if __name__ == '__main__':
    ...
    dataset = Dataset(args.path + args.dataset)
    model = DAICF(dataset.num_items, dataset.num_users, args)
    model.build_graph()
